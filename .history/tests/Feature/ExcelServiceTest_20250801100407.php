...
class ExcelServiceTest extends TestCase
{
    ...
    public function testImportTransactionsFromSheet()
    {
        ...
        // Debug zawartości pliku Excel przed uploadem
        $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($tempPath);
        $worksheet = $spreadsheet->getActiveSheet();
        echo "Zawartość pliku Excel przed uploadem:\n";
        ...
        $fileName = 'test_finances_' . date('Y-m-d_H-i-s') . '.xlsx';
        $uploadedFile = $this->googleDriveService->uploadFile($tempPath, $fileName, $this->folderId);
        
        if (!$uploadedFile) {
            $this->fail('Nie udało się przesłać pliku do Google Drive');
        }
        
        echo "✓ Przesłano plik: {$uploadedFile['name']} (ID: {$uploadedFile['id']})\n";

        // Wykonanie
        $result = $this->excelService->importTransactionsFromSheet($uploadedFile['id'], $user);
        ...
        echo "✓ Zaimportowano transakcje (liczba: {$result['imported_count']})\n";
        ...
        if ($uploadedFile) {
            $this->googleDriveService->deleteFile($uploadedFile['id']);
            echo "✓ Usunięto plik z Google Drive\n";
        }
    }
    ...
    public function testImportTransactionsWithInvalidData()
    {
        ...
        // Upload pliku do Google Drive
        $fileName = 'test_invalid_' . date('Y-m-d_H-i-s') . '.xlsx';
        $uploadedFile = $this->googleDriveService->uploadFile($tempPath, $fileName, $this->folderId);
    ...
    protected function tearDown(): void
    {
        ...
        // Usuń folder testowy z Google Drive
        if ($this->folderId) {
            try {
                $this->googleDriveService->deleteFile($this->folderId);
                echo "✓ Usunięto folder testowy z Google Drive\n";
            } catch (\Exception $e) {
                // Ignoruj błędy podczas sprzątania
            }
        }
    }
}
